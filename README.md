# BookMyTix - Flutter Flight Booking App UI Kit
----------

### Read documentation here
https://ilhammeidi.github.io/bookmytix-docs/

### Template Feature
- Flutter. Full source code (Dart)
- Premium designs. Simple and elegant theme color along with dark and light mode
- 40+ Screens with 100+ Widgets Ready-to-use UI screens, reusable and easy to customized.
- Various sample screens Home, Flights, Booking, Payment, and many more.
- Responsive design Support tablet screen
- Cross-platform support. iOS, Android and Mobile Web
- Clean architecture & modular design Fully customizable and developer-friendly code structure
- SVG vector icons, illustrations and font icons.
- Documentation. Well documented for customization
- And many more...

# Get The Full Version
![BookMyTix](https://ilhammeidi.github.io/bookmytix-docs/images/banner.jpg)

[GET IT NOW on Envato](https://codecanyon.net/item/bookmytix-flutter-flight-booking-app-template/57827751)

[SEE DEMO](https://bookmytix.indisains.com/)

### Installation and Deployment

 - Clone this project
 - Follow the instruction: [INSTALLATION](https://ilhammeidi.github.io/bookmytix-docs/#install)
 - Follow the instruction: [BUILD AND DEPLOYMENT](https://ilhammeidi.github.io/bookmytix-docs/#deploy)

### License
This project is licensed under the terms of the [MIT license](https://github.com/ilhammeidi/bookmytix-starter/blob/main/LICENSE).